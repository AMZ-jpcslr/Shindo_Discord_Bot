import { AutocompleteInteraction, ChannelType, ChatInputCommandInteraction, EmbedBuilder, PermissionFlagsBits, SlashCommandBuilder } from 'discord.js'
import { getRegions, getWarningReports, parseWarnings, radarImage, radarUrl, warningEmbed } from '../weather'
import { loadWeather, saveWeather, weatherState } from '../weather_notify'

export const data = new SlashCommandBuilder().setName('weather').setDescription('地域別の気象警報・雨の通知・雨雲レーダー')
    ...
    .addSubcommand(s => s.setName('set').setDescription('地域の通知を登録・変更（サーバー管理権限が必要）')
        ...
        .addChannelOption(o => o.setName('channel').setDescription('通知先').setRequired(true).addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement))
...
export async function autocomplete(interaction: AutocompleteInteraction): Promise<void> {
    try {
        ...
        const candidates = interaction.options.getSubcommand() === 'remove'
            ? Object.values(loadWeather()[interaction.guildId || ''] || {}).map(s => s.region)
            : await getRegions()
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (['set','remove'].includes(sub) && !interaction.memberPermissions?.has(PermissionFlagsBits.ManageGuild)) {
        await interaction.reply({ content: '通知設定の変更には「サーバー管理」権限が必要です。閲覧には /weather radar・now・status を使えます。', ephemeral: true }); return
    }
    await interaction.deferReply({ ephemeral: true })
    if (sub === 'status') {
        const subscriptions = loadWeather()[interaction.guildId] || {}
        ...
        await interaction.editReply({ embeds: [new EmbedBuilder().setTitle('気象通知の設定').setDescription(lines.join('\n\n') || '/weather set で地域と通知先を登録してください。').setFooter({ text: '警報: 約1分ごと / 降水予報: 約15分ごと・再通知は3時間以上間隔' })] }); return
    }
    const code = interaction.options.getString('area', true)
    if (sub === 'remove') {
        const config = loadWeather()
        const region = config[interaction.guildId]?.[code]
        if (!region) { await interaction.editReply('登録されていません。候補から登録地域を選んでください。'); return }
        delete config[interaction.guildId][code]
        saveWeather(config)
        await interaction.editReply(`${region.region.name} の気象・降水通知を停止しました。`); return
    }
    const region = (await getRegions()).find(r => r.code === code)
    if (!region) { await interaction.editReply('市区町村名を入力し、表示される候補から選んでください。'); return }
    if (sub === 'set') {
        const channel = await interaction.guild!.channels.fetch(interaction.options.getChannel('channel', true).id)
        const me = interaction.guild!.members.me || await interaction.guild!.members.fetchMe()
        if (!channel || ![ChannelType.GuildText, ChannelType.GuildAnnouncement].includes(channel.type) || !channel.permissionsFor(me)?.has([PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.EmbedLinks])) {
            await interaction.editReply('Botに通知先の「チャンネルを見る」「メッセージを送信」「埋め込みリンク」権限を付与してください。'); return
        }
        const config = loadWeather()
        config[interaction.guildId] ||= {}
        if (!config[interaction.guildId][code] && Object.keys(config[interaction.guildId]).length >= 10) { await interaction.editReply('1サーバーにつき10地域まで登録できます。/weather remove で整理してください。'); return }
        ...
        saveWeather(config)
        await interaction.editReply(`${region.name} → <#${channel.id}> に登録しました。約1分以内に初回確認します。\n/weather status で設定確認、/weather radar で雨雲、/weather remove で停止できます。\n再設定時、省略した項目は既定値に戻ります。`); return
    }
    if (sub === 'now') {
        await interaction.editReply({ embeds: [warningEmbed(region, parseWarnings(await getWarningReports(region.office), code, true))] }); return
    }
    const embed = new EmbedBuilder().setTitle(`${region.name}｜雨雲レーダー`).setURL(radarUrl(region)).setDescription('白丸は地域の中央付近です。色の凡例・アニメーションはタイトルのリンクから確認できます。').setFooter({ text: '雨雲: 気象庁 / 背景: 地理院タイル' })
    try {
