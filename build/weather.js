...
exports.warningNames = void 0;
exports.getRegions = getRegions;
exports.parseWarnings = parseWarnings;
exports.floodWarningReport = floodWarningReport;
exports.warningLabel = warningLabel;
exports.getWarningReports = getWarningReports;
exports.radarUrl = radarUrl;
exports.warningEmbed = warningEmbed;
exports.upcomingRain = upcomingRain;
exports.getRain = getRain;
exports.rainEmbed = rainEmbed;
exports.radarImage = radarImage;
...
async function getRegions() {
    ...
    loading = (async () => {
        const [areas, bounds] = await Promise.all([
            (0, http_1.json)('https://www.jma.go.jp/bosai/common/const/area.json'),
            (0, http_1.json)('https://www.jma.go.jp/bosai/common/const/class20relm.json'),
        ]);
...
function parseWarnings(data, region, advisories) {
    if (!Array.isArray(data) || !data.length)
        throw new Error('警報データの形式が不正です');
    const codes = new Set();
    let found = false;
    // Multiple reports cover different phenomena, not historical revisions.
    for (const report of data) {
        if (!report.warning || !Array.isArray(report.warning.class20Items) || !Number.isFinite(Date.parse(report.reportDatetime)))
            throw new Error('警報データの形式が不正です');
        ...
        for (const kind of area.kinds) {
            if (!kind.code || kind.status === '解除' || kind.status === '発表警報・注意報はなし')
                continue;
            const name = warningLabel(kind.code);
            // Unknown future codes are surfaced, never silently treated as safe.
            if (advisories || !name || !name.includes('注意報'))
                codes.add(kind.code);
        }
    }
    if (!found)
        throw new Error('指定地域が警報データにありません');
    return { codes: [...codes].sort(), time: data.map(r => r.reportDatetime).sort((a, b) => Date.parse(b) - Date.parse(a))[0] };
}
function floodWarningReport(data, office) {
    if (!Array.isArray(data))
        throw new Error('河川氾濫情報の形式が不正です');
    const areas = new Map();
    let time = '1970-01-01T00:00:00Z';
    for (const report of data) {
        if (report.status !== '通常' || report.infoType === '取消')
            continue;
        if (!Array.isArray(report.officeCodes) || !Array.isArray(report.class20Codes) || !report.item)
            throw new Error('河川氾濫情報の形式が不正です');
        if (!report.officeCodes.includes(office))
            continue;
        if (Date.parse(report.reportDatetime) > Date.parse(time))
            time = report.reportDatetime;
        // Codes 10/11 are cancellations. 20/21/22 are advisories; 30+ warnings.
        if (Number(report.item.code) < 20)
            continue;
...
function warningLabel(code) {
    ...
    return exports.warningNames[code] || `未対応の警報コード ${code}（気象庁で確認）`;
}
let floodCache;
async function getWarningReports(office) {
    if (!/^\d{6}$/.test(office))
        throw new Error('地域コードが不正です');
    if (!floodCache || Date.now() - floodCache.time >= 60_000) {
        floodCache = { time: Date.now(), promise: (0, http_1.json)('https://www.jma.go.jp/bosai/flood/data/r8/flood_xml.json') };
    }
    const [reports, floods] = await Promise.all([
        (0, http_1.json)(`https://www.jma.go.jp/bosai/warning/data/r8/${office}.json`), floodCache.promise,
    ]);
    if (!Array.isArray(reports) || !reports.length)
        throw new Error('警報データの形式が不正です');
    return [...reports, floodWarningReport(floods, office)];
}
function radarUrl(region) {
...
function warningEmbed(region, snapshot, previous) {
    const active = snapshot.codes.map(warningLabel);
    const cleared = (previous || []).filter(c => !snapshot.codes.includes(c)).map(warningLabel);
    return new discord_js_1.EmbedBuilder().setTitle(`${region.name}｜気象警報・注意報`).setColor(active.length ? 0xe67e22 : 0x2ecc71)
...
function upcomingRain(data, threshold, now = Date.now()) {
    const hourly = data?.hourly;
    if (!hourly || !Array.isArray(hourly.time) || !Array.isArray(hourly.precipitation) || hourly.time.length !== hourly.precipitation.length)
        throw new Error('降水予報の形式が不正です');
    ...
    if (candidates.length < 2 || candidates.some(v => v.mm === null || !Number.isFinite(v.mm)))
        throw new Error('新しい降水予報を取得できません');
    return candidates.find(v => v.mm >= threshold);
}
function getRain(region) {
    return (0, http_1.json)(`https://api.open-meteo.com/v1/forecast?latitude=${region.latitude}&longitude=${region.longitude}&hourly=precipitation&forecast_days=2&timeformat=unixtime&timezone=Asia%2FTokyo`);
}
function rainEmbed(region, event) {
    return new discord_js_1.EmbedBuilder().setTitle(`${region.name}｜降水予報`).setColor(0x3498db).setURL(radarUrl(region))
...
async function radarImage(region) {
    const times = await (0, http_1.json)('https://www.jma.go.jp/bosai/jmatile/data/nowc/targetTimes_N1.json');
    ...
    if (!latest)
        throw new Error('雨雲レーダー時刻がありません');
    ...
    if (Math.abs(Date.now() - time.getTime()) > 30 * 60_000)
        throw new Error('雨雲レーダー画像が古いため表示できません');
    const z = 8, n = 2 ** z;
    ...
    const tiles = await Promise.all([-1, 0, 1].flatMap(dy => [-1, 0, 1].map(async (dx) => {
        ...
        const inputs = await Promise.all(urls.map(async (url) => {
            const r = await (0, http_1.fetchWithTimeout)(url);
            if (!r.ok)
                throw new Error(`レーダータイル取得失敗: ${r.status}`);
            return Buffer.from(await r.arrayBuffer());
        }));
        return { input: await (0, sharp_1.default)(inputs[0]).composite([{ input: inputs[1] }]).png().toBuffer(), left: (dx + 1) * 256, top: (dy + 1) * 256 };
    })));
    ...
    const image = await (0, sharp_1.default)({ create: { width: 768, height: 768, channels: 4, background: '#ffffff' } }).composite(tiles).png().toBuffer();
    return { attachment: new discord_js_1.AttachmentBuilder(image, { name: 'radar.png' }), time };
}
